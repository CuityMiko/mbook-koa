...
Page({
  ...
  onUnload: function () {
    ...
    wx.setStorageSync('reader_setting', {
      allSliderValue: this.data.allSliderValue, // 控制当前章节，亮度，字体大小
      allFontFamily: this.data.allFontFamily, // 已经存在的字体列表
      currentSectionNum: this.data.currentSectionNum, // 当前小说的最新章节
  ...
  handletouchmove: function (event) {
    ...
    if (direction == 0) {
      if (currentIndex < self.data.maxPageNum) {
        self.setData({
          leftValue: self.data.leftValue - moreOrLessValue
        });
      }
    } else {
      if (currentIndex > 1) {
  ...
  showThisSection: function (event) {
    ...
    //根据章节id去得到章节内容
    wx.request({
      url: config.base_url + '/api/chapter/detail',
      method: 'GET',
      success(res) {
        try {
          ...
          self.setData({
            ...
            allSliderValue: {
              section: sectionNum,
              bright: self.data.allSliderValue.bright,
              font: self.data.allSliderValue.font
            }
          });
          wx.hideToast();
        } catch (e) {
      ...
      success: res => {
        if(res.data.ok){
          self.setData({ 
            ...
            'currentPageNum': res.data.page,
            'fationTitle': res.data.data.name,
