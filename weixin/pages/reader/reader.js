...
Page({
  data: {
    ...
    colorStyle: { content_bg: '#f8f7fc', styleNum: 1, slider_active_bg: '#757e87', slider_noactive_bg: '#dfdfdf', control_bg: '#ffffff', control_fontColor: '#616469' }, //1、2、3、4分别对应四种颜色模式
  ...
  onReady: function() {
    ...
    var maxPageNum = countPageNum(self.data.content, self.data.allSliderValue.font, self.data.lineHeight, (self.data.windows.windows_width-20), self.data.windows.windows_height, self.data.windows.pixelRatio);
  ...
  onLoad: function(options) {
    ...
    wx.getScreenBrightness({
      success: res => {
        self.setData({ 'allSliderValue.bright':  res.value})
      }
    })
  },
  //重新显示页面执行函数
  ...
  handletouchend: function() {
    ...
    if (hasRunTouchMove == false) {
      ...
      if (x && y && y >= (h - 100) && y <= (h + 100) && x >= (w - 100) && x <= (w + 100)) {
        self.setData({ control: { all: self.data.control.all === 0 ? 1 : 0, control_tab: 1, control_detail: 1, target: 'jingdu' }, isShowFontSelector: 0 });
        return;
      }
    }
    currentGesture = 0;
    ...
    if (isMoving == 0) {
      if (self.data.move_direction == 0) {
        if (currentIndex < self.data.maxPageNum) {
          targetLeftValue = (-1) * (self.data.windows.windows_width-20) * currentIndex;
        ...
        if (currentIndex > 1) {
          targetLeftValue = (-1) * (self.data.windows.windows_width-20) * (currentIndex - 2);
  ...
  sectionSliderChange: function(event) {
    var self = this;
    self.setData({ 'allSliderValue.section': event.detail.value });
  },
  brightSliderChange: function(event) {
    var self = this;
    var bright = event.detail.value / 100;
    wx.setScreenBrightness({ value: bright });
    self.setData({ 'allSliderValue.bright' : bright });
  },
  fontSliderChange: function(event) {
    var self = this;
    var maxPageNum = countPageNum(self.data.content, event.detail.value, self.data.lineHeight, (self.data.windows.windows_width-20), self.data.windows.windows_height, self.data.windows.pixelRatio);
    //重新计算分页
    self.setData({ 'allSliderValue.font': event.detail.value, 'maxPageNum': maxPageNum });
    console.log(event.detail.value, maxPageNum)
  },
  ...
  switchColorStyle: function(event) {
    ...
    switch (styleNum) {
      case '1':
        self.setData({ colorStyle: { content_bg: '#f8f7fc', styleNum: 1, slider_active_bg: '#757e87', slider_noactive_bg: '#dfdfdf', control_bg: '#ffffff', control_fontColor: '#616469' } });
        break;
      case '2':
        self.setData({ colorStyle: { content_bg: '#f6f0da', styleNum: 2, slider_active_bg: '#766f69', slider_noactive_bg: '#dad4c4', control_bg: '#f7f0de', control_fontColor: '#60594f' } });
        break;
      case '3':
        self.setData({ colorStyle: { content_bg: '#c0edc6', styleNum: 3, slider_active_bg: '#657568', slider_noactive_bg: '#aeccd6', control_bg: '#ccf1d0', control_fontColor: '#44644c' } });
        break;
      case '4':
        self.setData({ colorStyle: { content_bg: '#1d1c21', styleNum: 4, slider_active_bg: '#53565d', slider_noactive_bg: '#1a1e21', control_bg: '#10131a', control_fontColor: '#5b5e65' } });
        break;
    }
  },
    ...
    wx.request({
      ...
      success(res) {
        try {
          ...
          //重新排版
          var maxPageNum = countPageNum(newContent, self.data.allSliderValue.font, self.data.lineHeight, (self.data.windows.windows_width-20), self.data.windows.windows_height, self.data.windows.pixelRatio);
